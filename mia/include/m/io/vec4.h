 ...
 */
o_inline osize vec4_buffer(char *str, osize size, m_vec4 vec, bool colored, bool typed)
{
 ...
 * @param typed if true, prints with the vec<n> type (2..4)
 */
o_inline char *vec4_str(oobj parent, m_vec4 vec, bool colored, bool typed)
{
    return vecn_str(parent, vec.v, 4, colored, typed);
}
#define vec4_str_a(vec, colored, typed) vec4_str(o_arena(), (vec), (colored), (typed))
 ...
 * @param n the size
 */
o_inline osize vec4_print(m_vec4 vec)
{
 ...
 * @param n the size
 */
o_inline osize vec4_println(m_vec4 vec)
{
 ...
 */
o_inline bool vec4_json_read(m_vec4 *out_vec, oobj json_array)
{
 ...
 * @example json: [1.1, 2.2, 3.3, 4.4]
 */
o_inline oobj vec4_json_new(oobj parent, const char *name, m_vec4 vec)
{
