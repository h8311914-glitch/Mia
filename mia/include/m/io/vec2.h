 ...
 */
o_inline osize vec2_buffer(char *str, osize size, m_vec2 vec, bool colored, bool typed)
{
 ...
 * @param typed if true, prints with the vec<n> type (2..4)
 */
o_inline char *vec2_str(oobj parent, m_vec2 vec, bool colored, bool typed)
{
    return vecn_str(parent, vec.v, 2, colored, typed);
}
#define vec2_str_a(vec, colored, typed) vec2_str(o_arena(), (vec), (colored), (typed))
 ...
 * @param n the size
 */
o_inline osize vec2_print(m_vec2 vec)
{
 ...
 * @param n the size
 */
o_inline osize vec2_println(m_vec2 vec)
{
 ...
 */
o_inline bool vec2_json_read(m_vec2 *out_vec, oobj json_array)
{
 ...
 * @example json: [1.1, 2.2, 3.3, 4.4]
 */
o_inline oobj vec2_json_new(oobj parent, const char *name, m_vec2 vec)
{
