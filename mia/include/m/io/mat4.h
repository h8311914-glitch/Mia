 ...
 */
o_inline osize mat4_buffer(char *str, osize size, m_mat4 mat, bool colored, bool typed, bool multiline)
{
 ...
 * @note printed transposed, so it matches C row major, even if this lib is column major!
 */
o_inline char *mat4_str(oobj parent, m_mat4 mat, bool colored, bool typed, bool multiline)
{
    return matn_str(parent, mat.v, 4, colored, typed, multiline);
}
#define mat4_str_a(mat, colored, typed, multiline) \
    mat4_str(o_arena(), (mat), (colored), (typed), (multiline))
 ...
 * @note printed transposed, so it matches C row major, even if this lib is column major!
 */
o_inline osize mat4_print(m_mat4 mat, bool multiline)
{
 ...
 * @note printed transposed, so it matches C row major, even if this lib is column major!
 */
o_inline osize mat4_println(m_mat4 mat, bool multiline)
{
 ...
 * @note printed transposed, so it matches C row major, even if this lib is column major!
 */
o_inline bool mat4_json_read(m_mat4 *out_mat, oobj json_array)
{
 ...
 * @note printed transposed, so it matches C row major, even if this lib is column major!
 */
o_inline oobj mat4_json_new(oobj parent, const char *name, m_mat4 mat)
{
