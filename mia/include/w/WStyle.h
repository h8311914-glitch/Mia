...
/**
 * Creates a new WStyle object
 * @param parent to inherit from
 * @return The new object
 */
o_inline WStyle *WStyle_new(oobj parent)
{
    OObj_DECL_IMPL_NEW(WStyle, parent);
}

/**
 * Creates a new WStyle object, cloned from another WStyle
 * @param parent to inherit from
 * @param clone_from WStyle object to clone the values from, NULL for default
 * @return The new object
 */
WStyle *WStyle_new_clone(oobj parent, oobj clone_from);
...
/**
 * @param obj WStyle object
 * @return color applied on widget implementations, defaults to (1, 1, 1, 0.5)
 */
OObj_DECL_GETSET(WStyle, vec4, wobj_disabled_color)
 ...
 * @return size
 */
OObj_DECL_GETSET(WStyle, vec4, frame_border_size)
 ...
 * @return color
 */
OObj_DECL_GETSET(WStyle, vec4, icon_color)
