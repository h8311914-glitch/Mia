 ...
 * @param header_min_size header size (-1, -1 for vec2_(32, 10) as default)
 ...
 */
o_inline WWindow *WWindow_new(oobj parent, vec2 header_min_size)
 ...
 * Children are rendered behind the window.
 * The window consists of 2 WPane (header + body) which are also updated here.
...
/**
 * @param obj WWindow object
 * @return if true pointer got handled in that frame (in the window body or header)
 */
OObj_DECL_GET(WWindow, bool, active)
 ...
 *       The header does not inherit enabled flags from parents and is always called with enabled=true
 */
