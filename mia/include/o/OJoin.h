...
typedef struct {
    ...
    // this is a custom shared lock (uses OObjRoot), shared with OWeakjoin
    // Cause the OWeakjoin needs to be able to lock and access an OJoin (which may be deleted)
    oobj lock;

    // OList of parents (internal struct), if dropping to 0, the OJoin deletes itself
    oobj parents;

    // OList of weak parents (internal struct) (must be OWeak)
 ...
 * @return The new object
 */
o_inline OJoin *OJoin_new(oobj *parents, osize parents_size, struct o_allocator_i allocator, bool threadsafe)
{
    OJoin *self = OJoin_new_super(sizeof(OJoin), parents, parents_size, allocator);
    if (threadsafe) {
...
/**
 * Creates a new OJoin object, using o_allocator_heap_new as allocator
 * @param ... a va list of parents to inherit and join
 * @return The new object
 */
#define OJoin_new_heap(...) \
OJoin_new((oobj[]) { __VA_ARGS__, NULL }, -1, o_allocator_heap_new(), false)

/**
 * Creates a new OJoin object with the heap allocator and sets OObj_threadsafe to true (for this branch)
 * @param ... a list of parents to inherit and join
 * @return The new object
*/
#define OJoin_new_threadsafe(...) \
OJoin_new((oobj[]) { __VA_ARGS__, NULL }, -1, o_allocator_heap_new(), true)

/**
 * Creates a new OJoin object, using the allocator of the first parent
 * @param ... a va list of parents to inherit and join
 * @return The new object
 */
#define OJoin_new_share(...) \
OJoin_new((oobj[]) { __VA_ARGS__, NULL }, -1, OObj_allocator(O_VA_ARGS_FIRST(__VA_ARGS__)), false)
