 ...
 * The object "interface" matches the SDL io interface (and is the most common used sub object...)
 *
 * When subclassing, don't forget to set the destructor.
 *
 * Want a faster buffered stream?
 *    See OStreamBuffer
 *
 * Want to work with files? Have a look at "o/file.h"
 */
...
enum OStream_seek_whence {
    OStream_SET, // start of the stream
    OStream_CUR, // current position
    OStream_END, // end of the stream
    OStream_seek_ENUM_MAX,
};
 ...
 * @return true if the stream is in a valid state (ready to stream...)
 */
o_inline bool OStream_valid(oobj obj)
{
    if (!obj) {
        return false;
    }
 ...
 * @return the size of the stream object, <0 for unknown or error
 */
o_inline osize OStream_size(oobj obj)
{
    OObj_assert(obj, OStream);
    OStream *self = obj;
 ...
 * @return the resulting offset from the beginning of the stream or <0 for errors | seek not supported
 */
o_inline osize OStream_seek(oobj obj, osize offset, enum OStream_seek_whence whence)
{
    OObj_assert(obj, OStream);
    OStream *self = obj;
 ...
 * @return the number of elements read, which may be smaller than num!
 */
o_inline osize OStream_read_try(oobj obj, void *out_data, osize element_size, osize up_to_num)
{
    OObj_assert(obj, OStream);
    OStream *self = obj;
 ...
 * @return the number of elements written, or != num on failure
 */
o_inline osize OStream_write(oobj obj, const void *data, osize element_size, osize num)
{
    OObj_assert(obj, OStream);
    OStream *self = obj;
 ...
 * @return False if failed
 */
o_inline bool OStream_close(oobj obj)
{
    OObj_assert(obj, OStream);
    OStream *self = obj;
 ...
 * @note if using OSocketStream's, be aware that this function returns <num on a timeout
 */
 ...
 * Macro, so the user does not always includes "o/str.h", but the compiler is able to inline
 * @param obj OStream object
 * @param text static string to write (these that are embedded in code with "..")
 * @return The number of written bytes
 * @note needs to include "o/str.h"
 */
#define OStream_print_static(obj, text) OStream_write((obj), (text), 1, o_strlen_static(text))
 ...
 * @return The number of written bytes
 */
o_inline osize OStream_puts(oobj obj, const char *text)
{
 ...
 * Writes a line of text into the stream
 * @param obj OStream object
 * @param text string to append as line (null terminated...) STATIC string
 * @return The number of written bytes
 */
#define OStream_puts_static(obj, text) OStream_write((obj), text "\n", 1, o_strlen_static(text) + 1)
 ...
 */
o_inline int OStream_getchar(oobj obj)
{
    obyte c;
 ...
 * @param until_character_set multiple until characters null terminated ",." for eiter ',' or '.'
 ...
 * @return the characters read or -1 on error
 */
o_inline osize OStream_read_line_into(oobj obj, char *out_opt_data, osize max)
{
 ...
 * @return the string read or NULL if failed
 */
o_inline char *OStream_read_line(oobj obj, osize max)
{
 ...
 * @return the characters read or -1 on error
 */
o_inline osize OStream_read_isspace_into(oobj obj, int *out_opt_first_character_found,
                                         char *out_opt_data, osize max)
{
 ...
 * @return the string read or NULL if failed
 */
o_inline char *OStream_read_isspace(oobj obj, int *out_opt_first_character_found, osize max)
{
