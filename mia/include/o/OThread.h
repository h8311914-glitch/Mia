 ...
 *
 * @sa OQueue, OJoin_new_threadsafe to share data in a thread safe manner
 *
 * @note Needs MIA_OPTION_THREAD -> MIA_USE_THREAD
 */
 ...
 *       This OThread is passed into the thread fn. Be careful with race conditions.
 */
o_inline OThread *OThread_new(oobj parent, OObj__event_fn fn, const char *name)
{
    OObj_DECL_IMPL_NEW(OThread, parent, fn, name);
}
/**
 * Creates a new OThread object. Automatically calls OThread_run
 * @param parent to inherit from
 * @param fn the thread execution function
 * @param name the name for the thread
