 ...
 *       Functions with an `*_a` postfix are o_arena() allocated. So use with caution.
 *          o_arena() is typically set via a_app to be a frame based arena allocator with fixed bytes size.
 *
 * @sa view.h for fast parsing of const data
 */
 ...
 *            <=0 returns an empty string like ""
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_clone_sub(oobj obj, const char *to_clone, osize start, osize len)
{
    return o_str_clone_sub_realloc(obj, NULL, to_clone, start, len);
}
#define o_str_clone_sub_a(to_clone, start, len) o_str_clone_sub(o_arena(), (to_clone), (start), (len))
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_clone_range(oobj obj, const char *to_clone, struct o_str_range range)
{
    return o_str_clone_sub(obj, to_clone, range.start, range.len);
}
#define o_str_clone_range_a(to_clone, range) o_str_clone_range(o_arena(), (to_clone), (range))
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_clone_realloc(oobj obj, void *restrict mem, const char *to_clone)
{
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_clone(oobj obj, const char *to_clone)
{
    return o_str_clone_sub(obj, to_clone, 0, osize_MAX);
}
#define o_str_clone_a(to_clone) o_str_clone(o_arena(), (to_clone))
 ...
 * @threadsafe
 */
o_inline osize o_str_copy(char *dst, const char *to_clone)
{
    if(!dst || !to_clone) {
        return 0;
    }
...
#define o_strf_a(format, ...) o_strf(o_arena(), (format), __VA_ARGS__)
...
/**
 * Creates a new string from the given printf style format into the new allocated memory.
 * Uses o_arena as parent, so keep the resulting size small.
 * @param format printf style format (null terminated, must NOT be NULL)
 * @return The new string as resource from obj
 * @threadsafe (because allocations with objects are threadsafe)
 */
 ...
 * @param n Number os strings in the list (-1 for a null terminated list_of_strings)
...
#define o_str_join_a(list_of_strings, n, opt_seperator) o_str_join(o_arena(), (list_of_strings), (n), (opt_seperator))
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_cat(oobj obj, const char *a, const char *b)
{
    return o_str_join(obj, (char *[]) {(char *) a, (char *) b}, 2, NULL);
}
#define o_str_cat_a(a, b) o_str_cat(o_arena(), (a), (b))
...
#define o_str_rep_a(str, n, opt_seperator) o_str_rep(o_arena(), (str), (n), (opt_seperator))

/**
 * @return True if both strings equal (also in length), NULL safe (returns false)
 */
o_inline bool o_str_equals(const char *a, const char *b)
{
    if (!a || !b) {
        return false;
    }
...
/**
 * @returns the number of matching characters from the beginning, NULL safe (returns 0)
 */
osize o_str_count_matching(const char *a, const char *b);
...
#define o_str_find_all_a(str, search) o_str_find_all(o_arena(), (str), (search))

/**
 * Searches for the first occurrence of one character of set in the memory
 * @param str memory to check, may be NULL and returns -1 then
 * @param set characters in a set memory, may be NULL and returns -1 then
 * @return The position of a char from set or -1 if not found
 */
osize o_str_mem_find_set(const char *str, osize str_len, const char *set, osize set_len);

/**
 * Searches for the first occurrence of a character that is not in set, in the memory
 * @param str memory to check, may be NULL and returns -1 then
 * @param set characters in a set memory, may be NULL and returns -1 then
 * @return The position of a char from set or -1 if not found
 */
osize o_str_mem_find_set_inv(const char *str, osize str_len, const char *set, osize set_len);
 ...
 * @return The position of a char from set or -1 if not found
 */
o_inline osize o_str_find_set(const char *str, const char *set)
{
    return o_str_mem_find_set(str, o_strlen(str), set, o_strlen(set));
}
 ...
 * @return The position of search or -1 if not found
 */
o_inline osize o_str_find_set_inv(const char *str, const char *set)
{
    return o_str_mem_find_set_inv(str, o_strlen(str), set, o_strlen(set));
}
 ...
 * @return The position of search or -1 if not found
 */
o_inline osize o_str_mem_find_char(const char *str, osize str_len, char search)
{
    return o_str_mem_find_set(str, str_len, &search, 1);
}
 ...
 * @return The position of search or -1 if not found
 */
o_inline osize o_str_find_char(const char *str, char search)
{
 ...
 */
o_inline osize o_str_find_space(const char *str)
{
    return o_str_find_set(str, O_ISSPACE_SET);
}

/**
 * Searches for the first occurrence of a NONE whitespace (isspace) in the string
 * @param str String to check, may be NULL and returns -1 then
 * @return The position of the NONE whitespace character or -1 if not found
 */
o_inline osize o_str_find_space_inv(const char *str)
{
    return o_str_find_set_inv(str, O_ISSPACE_SET);
}
 ...
 * @return The position of search or -1 if not found
 */
o_inline osize o_str_find_back_char(const char *str, char search)
{
 ...
 * @return The position of the whitespace or -1 if not found
 */
o_inline osize o_str_find_back_space(const char *str)
{
    return o_str_find_back_set(str, O_ISSPACE_SET);
}

/**
 * Searches for the last occurrence of a NONE whitespace (isspace) in the string
 * @param str String to check, may be NULL and returns -1 then
 * @return The position of the NONE whitespace character or -1 if not found
 */
o_inline osize o_str_find_back_space_inv(const char *str)
{
    return o_str_find_back_set_inv(str, O_ISSPACE_SET);
}
...
#define o_str_replace_char_a(str, search, replace, out_opt_replaced) \
    o_str_replace_char(o_arena(), (str), (search), (replace), (out_opt_replaced))
...
#define o_str_replace_a(str, search, replace, out_opt_replaced) \
    o_str_replace(o_arena(), (str), (search), (replace), (out_opt_replaced))
...
#define o_str_trim_set_a(str, set, whence) o_str_trim_set(o_arena(), (str), (set), (whence))
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_trim_char(oobj obj, const char *str, char trim, enum o_str_trim_whence whence)
{
    return o_str_trim_set(obj, str, (char[2]) {trim, '\0'}, whence);
}
#define o_str_trim_char_a(str, trim, whence) o_str_trim_char(o_arena(), (str), (trim), (whence))
 ...
 * @threadsafe (because allocations with objects are threadsafe)
 */
o_inline char *o_str_trim(oobj obj, const char *str, enum o_str_trim_whence whence)
{
    return o_str_trim_set(obj, str, O_ISSPACE_SET, whence);
}
#define o_str_trim_a(str, whence) o_str_trim(o_arena(), (str), (whence))

/**
 * Splits a string into a list of ranges, allocated on object
 * terminated with a cleared element to 0 {0, 0}.
 * Call o_str_range_list_free to free.
 * @param obj to allocate the resource
...
#define o_str_split_ranges_a(str, split) o_str_split_ranges(o_arena(), (str), (split))
...
#define o_str_split_a(str, split) o_str_split(o_arena(), (str), (split))
...
#define o_str_tolower_a(str) o_str_tolower(o_arena(), (str))
...
#define o_str_toupper_a(str) o_str_toupper(o_arena(), (str))
...
#define o_str_escape_a(str) o_str_escape(o_arena(), (str))
...
#define o_str_unescape_a(str) o_str_unescape(o_arena(), (str))
...
#define o_str_base64_encode_a(data, size) o_str_base64_encode(o_arena(), (data), (size))
...
#define o_str_base64_decode_a(out_size, base64_str) o_str_base64_decode(o_arena(), (out_size), (base64_str))
 ...
 *       or just cleared (.len = 0) if not found.
...
#define o_str_pattern_find_a(pattern, str, out_opt_captures) \
    o_str_pattern_find(o_arena(), (pattern), (str), (out_opt_captures))
 ...
 * @param str to be matched
 * @param out_opt_captures if not NULL, a list of captures will be allocated on obj,
 *       or just cleared (.len = 0) if not matched.
 * @return true if matched
 * @sa OPattern, o_str_range_list_move, o_str_range_list_free
 */
bool o_str_pattern_match(oobj obj, const char *pattern, const char *str,
                         struct o_str_range_list *out_opt_captures);
#define o_str_pattern_match_a(pattern, str, out_opt_captures) \
    o_str_pattern_match(o_arena(), (pattern), (str), (out_opt_captures)
...
#define o_str_pattern_find_all_a(out_opt_matches, pattern, str, out_opt_captures_list) \
    o_str_pattern_find_all(o_arena(), (out_opt_matches), (pattern), (str), (out_opt_captures_list))
...
#define o_str_pattern_replace_a(pattern, str, replace, out_opt_replaced) \
    o_str_pattern_replace(o_arena(), (pattern), (str), (replace), (out_opt_replaced))
