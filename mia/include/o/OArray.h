 ...
 *
 * Want an array of type void* (or char, oobj, ...)?
 * > Have a look at OList <
 *
 * Operators:
 * o_num -> OArray_num
 * o_at -> OArray_at
 ...
 */
o_inline OArray *OArray_new(oobj parent, const void *opt_data, osize element_size, osize num)
{
 ...
 * @return The new object
 */
o_inline OArray *OArray_new_dyn(oobj parent, const void *opt_data, osize element_size, osize num,
 ...
 * @return The new object
 */
o_inline OArray *OArray_new_dyn_mode(oobj parent, const void *opt_data, osize element_size, osize num,
                             osize start_capacity, enum OArray_dyn_realloc_mode mode)
{
...
/**
 * Creates a new OArray object with a cloned string
 * @param parent to inherit from
 * @param string to copy from
 * @return The new object
 */
o_inline OArray *OArray_new_string(oobj parent, const char *string)
{
    if (!string) {
        string = "";
    }
 ...
 */
o_inline void *OArray_data_void(oobj obj)
{
 ...
 */
o_inline void *OArray_at_void(oobj obj, osize idx)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
 ...
 */
OObj_DECL_GET(OArray, osize, available_front)

/**
 * @param obj OArray object
 * @return remaining (capactity) elements at the back
 */
o_inline osize OArray_available(oobj obj)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
    return self->capacity - self->num - self->available_front;
}

/**
 * May o_memmove the data to apply the new offset, also may shrinken the num, it offset to large
 * @param obj OArray object
 * @param available_front from allocated memory to data, counted in elements
 */
void OArray_available_shift(oobj obj, osize available_front);
 ...
 * @param available_front from allocated memory to data, counted in elements
 */
void OArray_realloc(oobj obj, osize capacity, osize available_front);

/**
 * Resets the capacity to the current number of elements.
 * @param obj OArray object
 */
o_inline void OArray_capacity_fit(oobj obj)
{
 ...
 */
o_inline void OArray_dyn_realloc_fn_set(oobj obj, OArray__dyn_realloc_fn realloc)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
 ...
 * @ param obj OArray object
 */
o_inline void OArray_clear(oobj obj)
{
    OArray_resize(obj, 0);
}
 ...
 */
o_inline void *OArray_move(oobj obj, oobj into)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
 ...
 * @return a new OArray with data cloned
 */
o_inline OArray *OArray_clone(oobj obj)
{
 ...
 *                   The o__compare_fn user_data is set to obj OArray.
 ...
 */
void OArray_sort(oobj obj, o__compare_fn comperator);
 ...
 *       or "return m_sign(a-b)" for floats.
 */
osize OArray_search_sorted(oobj obj, const void *search_element, o__compare_fn comperator);

/**
 * Searches in the data array with brute force (valid for unsorted arrays)
 * @param obj OArray object
 * @param search_element the element to search the index for
 * @param comperator a comperator function, return ("a" - "b") to sort in ascending order (integer only)
 *                   <0: a is less than b.
 ...
 *       or "return m_sign(a-b)" for floats.
 */
osize OArray_search(oobj obj, const void *search_element, o__compare_fn comperator);
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void *OArray_push(oobj obj, const void *opt_element_data)
{
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void OArray_pop(oobj obj, void *out_opt_element_data)
{
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void *OArray_push_at(oobj obj, osize idx, const void *opt_element_data)
{
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void OArray_pop_at(oobj obj, osize idx, void *out_opt_element_data)
{
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void *OArray_push_front(oobj obj, const void *opt_element_data)
{
 ...
 *       DONT FORGET TO PASS A REFERENCE OF oobj ELEMENTS (would need oobj *)
 */
o_inline void OArray_pop_front(oobj obj, void *out_opt_element_data)
{
 ...
 */
o_inline osize OArray_append_string(oobj obj, const char *string)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
