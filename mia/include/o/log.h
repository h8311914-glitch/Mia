...
#define O_LOG_RING_SIZE 64

struct o_log_ring_entry {
    int level;
    char tag[8];
    char func[32];
    char msg[128];
...
enum o_log_level {
    O_LOG_TRACE, O_LOG_DEBUG, O_LOG_INFO, O_LOG_WARN, O_LOG_ERROR, O_LOG_WTF, O_LOG_ENUM_MAX
};

#if !defined(NDEBUG) && defined(MIA_OPTION_LOG_TRACE)
...
#if !defined(NDEBUG) && defined(MIA_OPTION_LOG_DEBUG)
...
#else
#define o_log_trace_s(func, ...) ((void)0)
#endif
...
#else
#define o_log_debug_s(func, ...) ((void)0)
#endif
...
/**
 * @return true if logs are copied to the ring buffer
 */
bool o_log_ring_enabled(void);

/**
 * @param set true to copy logs to the ring buffer
 */
void o_log_ring_enabled_set(bool set);

/**
 * Copys the current ring buffer of the last few (compact) logs
 * @param out_ring buffer of size O_LOG_RING_SIZE
 * @param out_pos oldest ring pos (next to be written)
 */
