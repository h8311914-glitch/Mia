 ...
 *  - class name as id, built up like: "OObj.Foo." for a class called Foo,
 *          or "OObj.Foo.Bar." for the class Bar, that inherits Foo
 *          classes delimted by a dot '.' (optional)
 *  - allocator, which is always inherit|copied by its parent
 *  - data
 ...
 *
 *  You can and should allocate with an object,
 ...
 *                     If the function needs to return an allocated buffer, use o_data_move
 *
 ...
 *      Like o_lock, o_alloc, ...
 *
 *  Using the same OObj is NOT THREADSAFE by default.
 *  To enable thread safe access to the children and memory, call OObj_threadsafe_set()
 *  This flag is automatically inherit by children of that object,
 *  so you can easily create a threadsafe branch of your tree.
 *  Not all allocators are usable for threadsafety and OObj_threadsafe_set will assert for it.
 *      Use the heap allocator. Apps default (module a) is a pool allcoator, which is indeed NOT THREADSAFE
 *  This system was designed to reduce locking in the oobj system,
 *      which invalidates caches and also remove the performance gain of a pool allocator, making things run slow.
 *
 ...
 *      #define Foo_ID OObj_ID "Foo."   // -> child of OObj so: "OObj.Foo."
 *      #define Bar_ID Foo_ID "Bar."    // -> child of Foo which is child of OObj so: "OObj.Foo.Bar.
 *  The delimiting dot '.' is optional
 */
#define OObj_ID "OObj."
...
typedef struct {
    ...
    /**
     * The allocator to create resources
    ...
    /**
     * If true, the mutex will be used to lock access to the hold data memory and children
     * Else, the current thread_id is asserted to match the OObj_thread_id() (from creation)
     */
    bool thread_safe;
#endif

    /** managed data buffers, will all be free'd in the deletor */
    void **data;
    osize data_capacity;
    osize data_num;
 ...
 *         So an object of type OObj.Foo.Bar. would be true for id "OObj.Foo.", but NOT vice versa.
 *         The delimiting dot '.' is optional
 * @threadsafe because ids must not be changed
 */
o_inline bool OObj_id_check(oobj obj, const char *id)
{
    assert(id && "id must not be NULL");
    OObj *self = (OObj *) obj;
    
    // uses strlen instead of o_strlen, because we are in a static header function
    bool check =  self != NULL && strncmp(self->id, id, strlen(id)) == 0;
    
    // lay a bread crump in case for a future OObj missmatch
    // see OObj_trace_breadcrumps
#if defined (MIA_OPTION_BREADCRUMPS) && !defined(NDEBUG)
    void OObj__breadcrump(OObj *self);
    if(check) {
        OObj__breadcrump(self);
    }
#endif
 ...
 * @note concatenates _ID to the type, so OObjData would be OObjData_ID which will be "OObj.Data."
 * @threadsafe because ids must not be changed
 */
 ...
 * @threadsafe because ids must not be changed
 */
o_inline bool OObj_id_check_wtf(oobj obj, const char *id, const char *func)
{
 ...
 * @threadsafe
 */
o_inline const char *OObj_id(oobj obj) {
 ...
 * @threadsafe
 */
o_inline const char *OObj_name(oobj obj) {
    OObj_assert(obj, OObj);
    OObj *self = obj;
 ...
 * @sa o_user_new0
 */
o_inline void *o_user_set(oobj obj, void *userdata)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
...
/**
 * Sets the userdata of this object to a new allocated resource.
 * Will exit the program if the allocation fails.
 * Also sets the memory to 0 with o_clear.
 * Not used by the library itself. (Internal objects may use it btw...)
 * Could be set for example to an allocated (on this object) struct of data.
 * @param obj OObj object
 * @param element_size Size of a single element (sizeof(...)).
 * @param num Numbers of elements to allocate.
 * @return the given allocated userdata
 * @example
 * struct data {int i;};
 * struct data *d = o_user_alloc0(obj, sizeof(*d), 1);
 * d->i = 12345;
 * @sa o_user_new0
 * @note Want more control like not cleared allocation "new" or realloc?
 *       Just use `o_user_set`
 *       Calls o_user_set internally
 */
#define o_user_alloc0(obj, element_size, num) \
    o_user_set((obj), o_alloc0((obj), (element_size), (num)))
 ...
 * @example
 * struct data {int i;};
 * struct data *d = o_user_new0(obj, *d, 1);
 * d->i = 12345;
 * @note Want more control like not cleared allocation "new" or realloc?
 *       Just use `o_user_set`
 *       Calls o_user_alloc0 internally, which calls o_user_set
 */
#define o_user_new0(obj, type_or_instance, count) \
    o_user_alloc0((obj), sizeof (type_or_instance), (count))
 ...
 * @note asserts that the operator is available
 */
o_inline void *o_at(oobj obj, osize idx)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
...
/**
 * @param obj: OObj object
 * @return id of the thread that created this OObj
 */
o_inline ou64 OObj_thread_id(oobj obj)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
...
/**
 * OObj's are NOT THREADSAFE by default.
 * Use this flag to activate save access to the children or data list.
 * The state of the flag is inherited by new OObj from the parent.
 * @param obj: OObj object
 * @return true if threadsafe access to children and data (default = false)
 */
o_inline bool OObj_threadsafe(oobj obj)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
 ...
 * @param obj: OObj object
 * @return true if threadsafe access to children and data (default = false)
 * @note asserts that the allocator is threadsafe
 */
...
// threading not enabled, just return 0 like o_thread_id()
#define OObj_thread_id(obj) ((ou64)0)

// only single thread, so always "safe"
#define OObj_threadsafe(obj) true
#define OObj_threadsafe_set(obj, set) true
 ...
 * @threadsafe
 */
o_inline oobj OObj_parent(oobj obj) {
    OObj_assert(obj, OObj);
    OObj *self = obj;
    return self->parent;
}
...
/**
 * @param obj OObj object
 * @return the internal data buffer list size
 * @threadsafe
 */
o_inline osize OObj_data_num(oobj obj)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
 ...
 * @threadsafe
 */
o_inline osize OObj_children_num(oobj obj)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
...
/**
 * Searches for the parent of the buffer in recursion.
 * @param obj The object to find the buffer in its object hierarchy
 * @param data The buffer to search
 * @param r_level: <=0 to only search in the given objects buffer list, without recursion.
                   else maximal recursion level, use oi32_MAX for all
 * @return the object which holds the buffer directly, or NULL if not found
 * @threadsafe
 */
struct oobj_opt OObj_data_search_parent(oobj obj, void *data, oi32 r_level);

/**
 * Moves a buffer pointer from the object hierarchy of "obj" into object "into".
 * Noop if data==NULL.
 * Asserts that both objects share the same allocator.
 * Asserts that data exists in the object hierarchy of obj.
 * @param obj The object hierarchy to search and move the buffer from
 * @param into The object to move a buffer into
 * @param data The buffer to move
 * @threadsafe
 */
...
/**
 * Searches for the first object with a matching name and id in the children in recursion.
...
/**
 * Collects the number of all objects in this tree, as well as resources and the tree depth.
 * These metrics may be useful for observing the app over time.
 * A constant grow of one of these values may report a soft leak (each frame new stuff is created for example)
 * @param obj The object tree to search in, NULL safe
 * @param out_oobjs Number of all oobjs in this tree, excluding obj itself (so may return 0)
 * @param out_datas Number of all allocated buffers in this tree
 * @param out_depth The depth of the tree
 * @note This function is NOT implemented in recursion. So a big depth level should not be a problem
 *       It will lock (and unlock...) the whole tree while traversing
 * @threadsafe
 */
void OObj_tree_metrics(oobj obj, osize *out_oobjs, osize *out_datas, osize *out_depth);

/**
 * prints logs all children in a recursion.
...
/**
 * prints all parents up to a root (OObjRoot or OJoin)
 * @param obj OObj object to print from
 * @threadsafe
 */
...
/**
 * prints all breadcrumps from its ring buffer.
 * Only prints if MIA_OPTION_BREADCRUMPS is set in debug builds.
 *     Else prints an info.
 * @threadsafe
 */
 ...
 */
o_inline struct o_allocator_i OObj_allocator(oobj obj)
{
    OObj_assert(obj, OObj);
    OObj *self = obj;
 ...
 * On a reallocation or free. The objects hierarchy is traversed to find the buffer
 ...
 *
 * @return the (re)allocated buffer or NULL, if it failed.
 * @threadsafe
 */
void *o_realloc_try(oobj obj, void *mem, osize element_size, osize num);

/**
 * Frees the buffer allocated by the given object.
 * @param obj The object to free from (uses its resource list)
 * @param mem the buffer to free (NULL safe)
 * @threadsafe
 */
o_inline void o_free(oobj obj, void *mem)
{
 ...
 * @param num  num <= 0 -> free (if mem != NULL).
 *
 * @return the (re)allocated buffer
 * @threadsafe
 */
o_inline void *o_realloc(oobj obj, void *mem, osize element_size, osize num)
{
 ...
 * @param element_size Size of a single element (sizeof(...)).
 * @param num Numbers of elements to allocate.
 * @return the allocated buffer or NULL, if it failed, or (element_size*num)==0.
 * @threadsafe
 */
o_inline void *o_alloc_try(oobj obj, osize element_size, osize num)
{
 ...
 * @threadsafe
 */
o_inline void *o_alloc0_try(oobj obj, osize element_size, osize num)
{
...
/**
 * Tries to allocate with the resource list of the given object.
 * Copies the data from to_clone into the allocated buffer.
 * @param obj The object to allocate from (uses its resource list)
 ...
 * @threadsafe
 */
o_inline void *o_alloc_clone_try(oobj obj, const void *to_clone, osize element_size, osize num)
{
    void *mem = o_realloc_try(obj, NULL, element_size, num);
    if (!mem)
        return NULL;
 ...
 * @param num Numbers of elements to allocate.
 *
 * @return the allocated buffer, or NULL if (element_size*num)==0
 * @threadsafe
 */
o_inline void *o_alloc(oobj obj, osize element_size, osize num)
{
 ...
 * Allocation with the resource list of the given object.
 * Will exit the program if the allocation fails.
 * Also sets the buffer to 0 with o_clea.
 * @param obj The object to allocate from (uses its resource list)
 * @param element_size Size of a single element (sizeof(...)).
 * @param num Numbers of elements to allocate.
 *
 ...
 * @threadsafe
 */
o_inline void *o_alloc0(oobj obj, osize element_size, osize num)
{
 ...
 * @threadsafe
 */
o_inline void *o_alloc_clone(oobj obj, const void *to_clone, osize element_size, osize num)
{
    void *mem = o_realloc(obj, NULL, element_size, num);
    if(mem) {
        // returns NULL if element_size*num is NULL
 ...
 * @param num Numbers of elements to allocate.
 *
 * @return the allocated buffer or NULL, if it failed.
 * @threadsafe
 */
...
o_data_move(O_VA_ARGS_FIRST(__VA_ARGS__), self, self);\
return self
 ...
 * @note some object *_new function may return NULL (in contrast to *_init, which NEVER return NULL!)
 */
o_inline OObj *OObj_new(oobj parent)
{
 ...
 */
#define o_del(obj) do { \
    oobj o__del__tmp_ = (obj);\
    (obj) = NULL;\
    OObj_del(o__del__tmp_);\
} while(0)
...
o_inline member_type obj_type ## _ ## member_name(oobj obj)          \
{                                                           \
...
o_inline member_type obj_type ## _ ## member_name ## _set(oobj obj, member_type set)     \
{                                                                               \
    OObj_assert(obj, obj_type);                                                 \
    obj_type *self = obj;                                                       \
...
o_inline member_type * obj_type ## _ ## member_name ## _ref(oobj obj)    \
{                                                               \
    OObj_assert(obj, obj_type);                                 \
    obj_type *self = obj;                                       \
    return &self->member_name;                                  \
