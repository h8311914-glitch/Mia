 ...
 * @param bound_tex already bound RTex to render to (or NULL for back)
 * @param program RProgram, already used
 * @param proj the camera projection to use (vp)
 */
typedef void (*RShader__render_fn)(oobj obj, oobj bound_tex, oobj program, int num, const struct r_proj* proj);
...
/**
 * Creates a new RShader object
 * @param parent to inherit from
 * @param program RProgram object to use, NULL safe (glsl shader program)
 * @param render_fn virtual render function
 * @return The new object
 */
o_inline RShader* RShader_new(oobj parent, oobj program, RShader__render_fn render_fn)
{
 ...
 */
o_inline void RShader_render(oobj obj, oobj buffer, oobj tex)
{
