 ...
 * @note
 */
o_inline RProgram *RProgram_new(oobj parent, char **shader_codes, int num_draw_buffers,
                       const char *source_tag_of_code, bool assume_valid)
{
 ...
 * Creates a new RProgram object
 * @param parent to inherit from
 * @param shader_file file of glsl code to compile, containing both vertex and fragment shader. (route path)
 ...
 */
o_inline ou32 RProgram_program(oobj obj)
{
    if(!obj) {
        return 0;
    }
 ...
 */
o_inline int RProgram_num_draw_buffers(oobj obj)
{
    if(!obj) {
        return 0;
    }
