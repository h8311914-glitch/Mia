 ...
 * @param seperator in units, according to mode or <0 for relative mode [-1:0]
 ...
 */
o_inline USplit *USplit_new(oobj parent, oobj child_a, oobj child_b, enum USplit_mode mode, float seperator, float spacing)
{
 ...
 * @note child a is always left or top and child b always right or bottom.
 *       These modes define which one is used for the seperator (from left or right)
 */
OObj_DECL_GETSET(USplit, enum USplit_mode, mode)

/**
 * @param obj USplit object
 * @return in units, according to mode
 *         or <0 for relative mode [-1:0]
 */
 ...
 * @note if true (default is false), spacing may grow to generate the even sizes
 */
