...
vec2 WImgPicker__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    // badge
    box->uv_rect = u_atlas_rect(atlas, sprite);
    box->rect = u_rect_new_lt(m_2(badge_lt), m_2(size_badge));
    box->fx = self->pressed ? self->badge_pressed_color : self->badge_color;


    // WImg
    vec2 full_size = WImg__v_update(self, lt, min_size, enabled, theme, pointer_fn);
    ...
    vec4 picker_rect = u_rect_new_lt(m_2(lt), m_2(picker_field));

    vec2 pp = pointer.pos.xy;
    if (u_rect_contains(picker_rect, pp)) {
