...
vec2 WProgress__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    box->uv_rect = u_atlas_rect(WTheme_atlas(theme), WTheme_WHITE);
    box->rect = u_rect_new_lt(m_2(progress_lt), m_2(progress_size));
