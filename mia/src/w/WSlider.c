...
vec2 WSlider__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    vec4 bar_color = WObj_enabled_apply_color(self, enabled, self->bar_color);
    vec4 badge_color = WObj_enabled_apply_color(self, enabled, self->badge_color);
    ...
    struct r_box *boxes = WObj__alloc_boxes(self, theme, 3+1);
    ...
    b = &boxes[0];
    b->uv_rect = u_atlas_rect(atlas, sprite + 0);
    sprite_size = u_atlas_size(atlas, sprite + 0);
    b->rect = u_rect_new_lt(m_2(sprite_lt), m_2(sprite_size));
    b->fx = bar_color;
    sprite_lt = vec2_(sprite_lt.x + sprite_size.x * hs.x,
                      sprite_lt.y - sprite_size.y * hs.y);

    b = &boxes[1];
    b->uv_rect = u_atlas_rect(atlas, sprite + 1);
    if(self->vertical) {
    ...
    b = &boxes[2];
    b->uv_rect = u_atlas_rect(atlas, sprite + 2);
    ...
    // badge
    b = &boxes[3];
    b->uv_rect = u_atlas_rect(atlas, sprite + 3);
    sprite_size = u_atlas_size(atlas, sprite + 3);
    b->rect = u_rect_new_lt(m_2(badge_lt), m_2(sprite_size));
    b->fx = self->pressed? self->badge_pressed_color : badge_color;
    ...
    vec4 slider_rect = u_rect_new_lt(m_2(lt), m_2(slider_field));

    vec2 pp = pointer.pos.xy;
    if(u_rect_contains(slider_rect, pp)) {
