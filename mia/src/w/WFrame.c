...
vec2 WFrame__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    OObj_assert(obj, WFrame);
    WFrame *self = obj;

    vec4 border_color = WObj_enabled_apply_color(self, enabled, self->border_color);

    vec2 label_size = WObj_update(self->label, lt, vec2_(0), enabled, theme, pointer_fn);

    vec2 inner_lt = lt;
    inner_lt.x += self->border_size.v0;
    inner_lt.y += o_max(label_size.y, self->border_size.v1);

    vec2 inner_min_size = vec2_(
            o_max(label_size.x, min_size.x) - self->border_size.v0 - self->border_size.v2,

            min_size.y - o_max(label_size.y, self->border_size.v1) - self->border_size.v3
    );

    vec2 inner_size = WObj__update_children_stacked(self, inner_lt, inner_min_size, enabled, theme, pointer_fn);

    // theme the boxes
    struct r_box *boxes = WObj__alloc_boxes(self, theme, 4);

    for(int i=0;i<4;i++) {
        boxes[i].uv_rect = u_atlas_rect(WTheme_atlas(theme), WTheme_WHITE);
        boxes[i].fx = border_color;
    }
    ...
    // left line
    if (self->border_size.v0 > 0) {
        boxes[0].rect = u_rect_new_rt(
                inner_lt.x,
        ...
        boxes[0].rect = u_rect_new_hidden();
    }

    // top line
    if (self->border_size.v1 > 0) {
        boxes[1].rect = u_rect_new_lb(
        ...
        boxes[1].rect = u_rect_new_hidden();
    }

    // right line
    if (self->border_size.v2 > 0) {
        boxes[2].rect = u_rect_new_lt(
        ...
        boxes[2].rect = u_rect_new_hidden();
    }

    // bottom line
    if (self->border_size.v3 > 0) {
        boxes[3].rect = u_rect_new_lt(
                inner_lt.x,
        ...
        boxes[3].rect = u_rect_new_hidden();
    }



    vec2 full_size = vec2_(
        o_max(label_size.x, self->border_size.v0 + inner_size.x) + self->border_size.v2,
        
        o_max(label_size.y, self->border_size.v1) + inner_size.y + self->border_size.v3
    );
