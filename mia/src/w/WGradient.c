...
static int determine_decimal_places(float step)
{
...
static int determine_num_width(float min, float max)
{
    min = o_abs(min);
    max = o_abs(max);
    max = o_max(min, max);
...
static float num_to_progress(WGradient *self, float num)
{
...
static float progress_to_num(WGradient *self, float progress)
{
    progress = o_clamp(progress, 0.0f, 1.0f);
...
static void imgpicker_event(oobj obj)
{
...
WGradient *WGradient_init(oobj obj, oobj parent, vec4 uv_rect, float min, float max, float step, const char *opt_label)
{
    ...
    oobj box = WBox_new_h(self->frame);
...
vec2 WGradient__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
