...
WStyle *WStyle_init(oobj obj, oobj parent)
{
    ...
    self->wobj_disabled_color = vec4_(1, 1, 1, 0.5);
    ...
    self->frame_border_size = vec4_(1);
...
{
    WStyle *self = WStyle_new(parent);
    if (!clone_from) {
        return self;
    }
    OObj_assert(clone_from, WStyle);
    WStyle *clone = clone_from;

    // WStyle only contains static data, so we just copy the whole thing
    // But reset the OObj super, so only the remaining is copied.
    // Even better would be to only memcpy with offsets, but its easier and enough this way.
    OObj super_tmp = self->super;
    *self = *clone;
    self->super = super_tmp;
    return self;
}
