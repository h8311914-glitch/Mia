...
static int num_determine_decimal_places(float step)
{
...
static int num_determine_num_width(float min, float max)
{
    min = o_abs(min);
    max = o_abs(max);
    max = o_max(min, max);
...
static float num_num_to_progress(WNum *self, float num)
{
...
static float num_progress_to_num(WNum *self, float progress)
{
    progress = o_clamp(progress, 0.0f, 1.0f);
...
static void num_sliderdrag_event(oobj obj)
{
...
WNum *WNum_init(oobj obj, oobj parent, float min, float max, float step, const char *opt_label)
{
    ...
    oobj box = WBox_new(self->frame, WBox_H_WEIGHTS);
...
vec2 WNum__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
