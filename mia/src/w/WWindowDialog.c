...
static void on_btn_cancel_clicked(oobj btn)
{
...
static void on_btn_ok_clicked(oobj btn)
{
    oobj dialog_obj = o_user(btn);
    OObj_assert(dialog_obj, WWindowDialog);
    WWindowDialog *self = dialog_obj;
...
WWindowDialog *WWindowDialog_init(oobj obj, oobj parent, const char *title, bool show_cancel, bool show_ok)
{
    ...
    oobj txt = WTextShadow_new(WWindow_header(self), title);

    oobj grid = WGrid_new(WWindow_body(self), 2, 2, vec2_(-1));
    
    oobj hbox = WBox_new_h(grid);
...
vec2 WWindowDialog__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    OObj_assert(obj, WWindow);
    WWindowDialog *self = obj;

    vec2 size = WWindow__v_update(obj, lt, min_size, enabled, theme, pointer_fn);
    
    if(self->bg_maximize) {
        struct r_box *box = WObj_gen_boxes(self->bg);
        // m_MAX is to large...
        box->rect = u_rect_new(-1E5, -1E5, +2E5, +2E5);
    }
    ...
    struct r_box *box = WObj_gen_boxes(self->bg);
    if(box && u_rect_contains(box->rect, p.pos.xy)) {
        a_pointer_handled(0, 0);
    }
