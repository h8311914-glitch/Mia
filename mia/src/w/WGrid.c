...
static vec4 update_child(WGrid *self, oobj child, vec2 cell_lt, vec2 cell_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    vec2 child_size = WObj_update(child, cell_lt, min_size, enabled, theme, pointer_fn);
    
    return vec4_(m_2(cell_lt), m_2(child_size));
}

static vec4 get_cell_lt_size(WGrid *self, vec2 lt, int col, int row, int w, int h,
                      const float *col_sizes, const float *row_sizes) {
    lt.x += vecn_sum(col_sizes, col) + self->spacing.x * col;
    lt.y += vecn_sum(row_sizes, row) + self->spacing.y * row;
...
static vec2 get_cell_child_size(WGrid *self, vec2 child_size, int col, int row, int w, int h,
                         const float *col_sizes, const float *row_sizes)
{
...
vec2 WGrid__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    o_data_move(self, container, list);
    ...
    {
        float weight_sum = 0;
        float room = min_size.x - self->spacing.x * (self->cols-1);
    ...
    {
        float weight_sum = 0;
        float room = min_size.y - self->spacing.y * (self->rows-1);
    ...
    while(*it) {
        ...
        vec4 child_lt_size = update_child(self, child, cell_lt_size.xy, cell_lt_size.zw, enabled, theme, pointer_fn);

        vec2 rb = vec2_(child_lt_size.x + child_lt_size.v2,
                        child_lt_size.y + child_lt_size.v3);

        max_rb.x = o_max(max_rb.x, rb.x);
        max_rb.y = o_max(max_rb.y, rb.y);
    ...
    vec2 grid_used_child_size = vec2_(max_rb.x - lt.x, max_rb.y - lt.y);
    vec2 grid_cell_sum_size;
    grid_cell_sum_size.x = vecn_sum(self->col_sizes, self->cols) + self->spacing.x * (self->cols-1);
    grid_cell_sum_size.y = vecn_sum(self->row_sizes, self->rows) + self->spacing.y * (self->rows-1);
...
void WGrid_resize(oobj obj, int cols, int rows)
{
    ...
    float *col_sizes = o_new0(self, float, (cols + rows)*3);
...
WObj **WGrid_at_all(oobj obj, int col, int row, bool only_lt, osize *opt_out_num)
{
    ...
    WGrid *self = obj;
    osize children_num;
    WObj **children = WObj_list(self, &children_num);
    WObj **list = o_new0(self, *list, children_num);
    osize list_num = 0;
    
    for(WObj **it=children; *it; it++) {
        ivec4 cell = WGrid_child_cell(*it);
        if(only_lt) {
            if(cell.x == col && cell.y == row) {
                list[list_num++] = *it;
            }
        } else {
            int cell_x2 = cell.x+cell.width-1;
            int cell_y2 = cell.y+cell.height-1;
            if(cell.x <= col && col <= cell_x2
                && cell.y <= row && row <= cell_y2) {
                list[list_num++] = *it;
            }
        }
    }
    
    o_free(self, children);
...
{
    WObj **list = WGrid_at_all(obj, col, row, only_lt, NULL);
    oobj found = *list;
    o_free(obj, list);
    return oobj_opt(found);
}
