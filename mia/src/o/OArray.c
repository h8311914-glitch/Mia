...
o_inline void reset_available_front(OArray *self, osize new_valid_available_front)
{
    self->available_front = new_valid_available_front;
    self->data = &self->memory[self->available_front * self->element_size];
}

o_inline void set_termination_element(OArray *self)
{
...
static OArray *array_init_base(oobj obj, oobj parent, const void *opt_data, osize element_size, osize num,
                               osize start_capacity, osize available_front, OArray__dyn_realloc_fn realloc_fn)
{
    OArray *self = obj;
    ...
    available_front = o_clamp(available_front, 0, start_capacity-num);
    ...
    reset_available_front(self, available_front);
                    ...
                    osize start_capacity, enum OArray_dyn_realloc_mode mode)
{
    ...
    return array_init_base(obj, parent, opt_data, element_size, num,
                           start_capacity, available_front, realloc_fn);
}
...
{
    // doubled capacity at front, available_front is as much to right as possible
    osize cap = min_needed * 2;
...
{
    // doubled capacity at front, available_front layed so that the data is nearly centered
    osize cap = min_needed * 2;
    OArray_realloc(obj, o_max(cap, OArray_capacity(obj)),
                   min_needed - OArray_num(obj) / 2 + append_front - append_back);
}
...
void OArray_available_shift(oobj obj, osize available_front)
{
    ...
    available_front = o_max(available_front, 0);
    if (self->available_front == available_front) {
        return;
    }
    osize new_num = self->capacity - available_front;
    if (new_num > 0) {
        self->num = o_min(new_num, self->num);
        self->available_front = available_front;
        o_memmove(&self->memory[self->available_front * self->element_size], self->data, self->element_size, self->num);
    } else {
        self->num = 0;
        self->available_front = self->capacity;
    }
    reset_available_front(self, self->available_front);
...
void OArray_realloc(oobj obj, osize capacity, osize available_front)
{
    ...
    if (capacity < self->capacity) {
        ...
        OArray_available_shift(self, available_front);
    }
    ...
    // widen array, or available_front only, o_memmove after reallocation
    OArray_available_shift(self, available_front);
}
...
void OArray_resize(oobj obj, osize num)
{
    ...
    // adding elements in capacity or popping
...
void OArray_resize_front(oobj obj, osize num)
{
    ...
    if (add <= available) {
        self->num = num;
        reset_available_front(self, self->available_front - add);
        // maybe no need for set_termination_element, just to be sure...
        set_termination_element(self);
        return;
    }
...
void OArray_sort(oobj obj, o__compare_fn comperator)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
    o_qsort(self->data, self->element_size, self->num, self, comperator);
}
...
osize OArray_search_sorted(oobj obj, const void *search_element, o__compare_fn comperator)
{
    OObj_assert(obj, OArray);
    OArray *self = obj;
    const obyte *searched = (const obyte *) o_bsearch(search_element, self->data,
                                                      self->element_size, self->num, self, comperator);
    if (!searched) {
        return -1;
    }
...
osize OArray_search(oobj obj, const void *search_element, o__compare_fn comperator)
{
    ...
    for (osize i = 0; i < self->num; i++) {
        if (comperator(self, search_element, OArray_at_void(self, i)) == 0) {
            return i;
        }
    }
    return -1;
