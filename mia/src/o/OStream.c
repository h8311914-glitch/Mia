                              ...
                              const char *until_character_set)
{
    ...
    osize until_character_set_len = o_strlen(until_character_set);
    do {
        ...
        len++;
        if (o_str_mem_find_char(until_character_set, until_character_set_len, (char) r) >= 0) {
                         ...
                         osize max, const char *until_character_set)
{
    ...
    for (osize len=0; len<max; len++) {
        ...
        char c = (char) r;
        OArray_push(data, &c);
        if (o_str_mem_find_char(until_character_set, until_character_set_len, c) >= 0) {
            o_opt_set(out_opt_until_character_found, r);
            break;
        }
    }
                            ...
                            const char *all_character_set)
{
    ...
    osize len = 0;
    osize all_character_set_len = o_strlen(all_character_set);
    do {
        int r = OStream_getchar(obj);
        if (r < 0) {
            return 0;
        ...
        len++;
        if (o_str_mem_find_char(all_character_set, all_character_set_len, (char) r) < 0) {
...
char *OStream_read_all(oobj obj, int *out_opt_first_character_found, osize max, const char *all_character_set)
{
    ...
    for (osize len=0; len<max; len++) {
        ...
        char c = (char) r;
        if (o_str_mem_find_char(all_character_set, all_character_set_len, c) < 0) {
            o_opt_set(out_opt_first_character_found, r);
            break;
        }
        OArray_push(data, &c);
