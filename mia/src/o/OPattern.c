...
o_inline void OPattern_group_set(struct OPattern_group *self, obyte position, bool set)
{
...
static void OPattern_group_set_range(struct OPattern_group *self, obyte from, obyte to)
{
...
static void OPattern_group_set_set(struct OPattern_group *self, const char *set, int set_size)
{
...
static void OPattern_group_inv(struct OPattern_group *self)
{
    for (int i = 0; i < 32; i++) {
...
static void OPattern_group_or(struct OPattern_group *self, const struct OPattern_group *or)
{
    for (int i = 0; i < 32; i++) {
...
static void OPattern_group_set_dot(struct OPattern_group *self)
{
    for (int i = 0; i < 32; i++) {
        self->flags[i] = 0xFF;
    }
...
static void OPattern_group_set_a(struct OPattern_group *self)
{
    OPattern_group_set_range(self, 'A', 'Z');
    OPattern_group_set_range(self, 'a', 'z');
}

static void OPattern_group_set_c(struct OPattern_group *self)
{
    OPattern_group_set_set(self, SET_c, sizeof(SET_c));
}

static void OPattern_group_set_d(struct OPattern_group *self)
{
    OPattern_group_set_range(self, '0', '9');
}

static void OPattern_group_set_l(struct OPattern_group *self)
{
    OPattern_group_set_range(self, 'a', 'z');
}

static void OPattern_group_set_p(struct OPattern_group *self)
{
    OPattern_group_set_set(self, SET_p, sizeof(SET_p));
}

static void OPattern_group_set_s(struct OPattern_group *self)
{
    OPattern_group_set_set(self, SET_s, sizeof(SET_s));
}

static void OPattern_group_set_u(struct OPattern_group *self)
{
    OPattern_group_set_range(self, 'A', 'Z');
}

static void OPattern_group_set_w(struct OPattern_group *self)
{
    OPattern_group_set_range(self, '0', '9');
    OPattern_group_set_range(self, 'A', 'Z');
    OPattern_group_set_range(self, 'a', 'z');
...
static void OPattern_group_set_x(struct OPattern_group *self)
{
    OPattern_group_set_range(self, '0', '9');
...
static void OPattern_group_set_z(struct OPattern_group *self)
{
...
static void parse(OPattern *self, const char *pattern)
{
    ...
    for (osize p_idx = 0; p_idx < pattern_len; p_idx++) {
        ...
        if (!in_group) {
            ...
            obyte next = (obyte) pattern[p_idx + 1];
            if (next == '+') {
                current->mode = OPattern_group_PLUS;
                p_idx++;
            } else if (next == '*' || next == '-') {
                current->mode = OPattern_group_MUL;
                p_idx++;
            } else if (next == '?') {
                current->mode = OPattern_group_OPT;
                p_idx++;
            } else {
                current->mode = OPattern_group_SINGLE;
            }
...
static struct o_str_range find_at(OPattern *self, const void *data, osize data_size,
    ...
    for (;;) {
        ...
        if (pos >= data_size && group_idx < self->groups_size) {
            ...
            for (osize gi = group_idx; gi < self->groups_size; gi++) {
                if (self->groups[gi].mode != OPattern_group_MUL
                    && self->groups[gi].mode != OPattern_group_OPT
                    && !(self->groups[gi].mode == OPattern_group_PLUS && group_cnt > 0)) {
                    valid = false;
                    break;
                }
            }
        ...
        if (!valid) {
            if ((group_cnt > 0 && self->groups[group_idx].mode == OPattern_group_PLUS)
                || self->groups[group_idx].mode == OPattern_group_MUL
                || self->groups[group_idx].mode == OPattern_group_OPT) {
        ...
        if (self->groups[group_idx].mode == OPattern_group_SINGLE) {
        ...
        if (self->groups[group_idx].mode == OPattern_group_PLUS) {
        ...
        if (self->groups[group_idx].mode == OPattern_group_MUL) {
            if (!used_repstuff) {
                // add rep and check next character, same group
                reppoint.pos = pos;
                reppoint.group = group_idx;
        ...
        if (self->groups[group_idx].mode == OPattern_group_OPT) {
            if (!used_repstuff) {
