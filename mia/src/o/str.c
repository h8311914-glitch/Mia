...
{
    o_data_move(list->parent, into, list->list);
    list->parent = into;
}
...
void o_str_range_list_list_move(struct o_str_range_list_list *list_list, oobj into)
{
    ...
    o_data_move(list_list->parent, into, list_list->list);
...
char *o_str_clone_sub_realloc(oobj obj, void *restrict mem, const char *to_clone, osize start, osize len)
{
    ...
    max_size = o_max(0, max_size);
    len = o_clamp(len, 0, max_size);
...
struct o_str_range_list o_str_split_ranges(oobj obj, const char *str, const char *split)
{
    ...
    osize max_splits = o_str_count(str, split) + 1;

    // null terminated list
    struct o_str_range *ranges = o_new0(obj, struct o_str_range, max_splits + 1);
    ...
    osize start = 0;
    osize splits = 0;
    for (osize i = 0; i < max_splits; i++) {
        osize len = o_str_find(str + start, split);
        if (len==0) {
            // split at start, ignoring
            start++;
            continue;
        }
        if (len <0) {
            // no remaining splits found, checking for end of string
            len = o_strlen(str + start);
            if (len > 0) {
 ...
 */
static obyte str_obyte_from_hex(const char *hex_byte_chars)
{
            ...
            switch (*str) {
                ...
                case 'f':
                    *it++ = '\f';
                    break;
                case 'x':
                    *it++ = (char) str_obyte_from_hex(str);
                    str += 2;
                    break;
                default:
                                      ...
                                      struct o_str_range_list *out_opt_captures)
{
    ...
    struct o_str_range *captures_list = NULL;
    if(out_opt_captures) {
        captures = o_str_range_list_new(obj, OPattern_captures(p));
        captures_list = captures.list;
    }
    struct o_str_range range = OPattern_find(p, str, captures_list);
    if (out_opt_captures) {
        if(range.len <= 0) {
            o_str_range_list_free(&captures);
            captures = (struct o_str_range_list) {0};
        }
                         ...
                         struct o_str_range_list *out_opt_captures)
{
    ...
    bool matched = OPattern_match(p, str, captures_list);
    if (out_opt_captures) {
        if(!matched) {
            o_str_range_list_free(&captures);
            captures = (struct o_str_range_list) {0};
