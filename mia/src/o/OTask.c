...
static void run_once(OTask *self) {
    o_lock_block(self) {
...
static float finished_task(oobj task, int run)
{
    return OTask_FINISH;
}
...
{
    OTask *self = OTask_new(parent, finished_task, false);
    self->progress = OTask_FINISH;
    return self;
}
...
static void future_run(oobj future)
{
