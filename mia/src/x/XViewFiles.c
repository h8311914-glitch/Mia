...
static oobj dir_stack_clone(oobj parent, oobj stack)
{
    oobj clone = OList_new_ex(parent, NULL, o_num(stack), OList_capacity(stack), OArray_REALLOC_DEFAULT);
    for (osize i = 0; i < o_num(stack); i++) {
        char *src = o_at(stack, i);
        char **dst = (char **) OList_at_ref(clone, i);
        *dst = o_str_clone(clone, src);
    }
    return clone;
}

static void dir_clicked(oobj btn)
{
...
static void file_clicked(oobj btn)
{
    XViewFiles *self = o_user(btn);
    struct oobj_opt text = OObj_find(btn, WText, NULL, oi32_MAX);
    assert(text.o);
...
static void files_page_prev(oobj btn)
{
    XViewFiles *self = o_user(btn);
    self->w.f.page--;
}

static void files_page_next(oobj btn)
{
    XViewFiles *self = o_user(btn);
    self->w.f.page++;
}
...
static void append_page_btn(XViewFiles *self, oobj box, int page, int pages, bool prev)
{
    ...
    oobj h = WBox_new_h(btn);
    WBox_spacing_set(h, vec2_(4));
    WIcon_new(h, prev ? WTheme_ICON_ARROW_UP : WTheme_ICON_ARROW_DOWN);
    char buf[32];
    o_strf_buf(buf, "PAGE %i / %i", page, pages);
    WText_new(h, buf);
}

static void append_dir(XViewFiles *self, oobj box, const char *name)
{
    oobj btn = WBtn_new(box);
    ...
    oobj name_text = WText_new(h, name);

    // prevent from to large names
    WText_wrap_size_set(name_text, vec2_(112, 0));
    WText_wrap_set(name_text, WText_wrap_ANY);
}

static void append_file(XViewFiles *self, oobj box, const char *name)
{
    oobj btn = WBtn_new(box);
    WBtn_style_set(btn, WBtn_FLAT);
...
static void files_update(oobj view, oobj tex, float dt)
{
    ...
    if (!recreate) {
        ...
        oobj box = WBox_new_v(gui);
        WBox_spacing_set(box, vec2_(0));

        if (dir_changed) {
            o_del(self->w.cached_dirs);
            o_del(self->w.cached_files);
            self->w.cached_dirs = o_file_list(self, self->w.f.last_dir, o_file_info_type_DIR, NULL);
            self->w.cached_files = o_file_list(self, self->w.f.last_dir, o_file_info_type_FILE, NULL);
        }
        ...
        osize entry = 0;
        osize dirs_num = OList_num(dirs);
        char **dirs_v = (char **) OList_list(dirs);
        for (osize i = 0; i < dirs_num; i++, entry++) {
        ...
        char **files_v = (char **) OList_list(files);
        for (osize i = 0; i < files_num; i++, entry++) {
            if (entry >= start_entry && entry < end_entry) {
                char *file = files_v[i];
    ...
    vec2 size = WTheme_update(self->w.f.theme, self->w.f.gui, vec2_(0), vec2_(128));
    ...
    vec4 cam_rect = u_rect_new(0, 0, m_2(size));
    cam_rect.xy = vec2_sub(cam_rect.xy, border);
    cam_rect.zw = vec2_add(cam_rect.zw, 2*border);
    self->w.scroll.cam_limits_rect = cam_rect;
}

static void files_render(oobj view, oobj tex, float dt)
{
    XViewFiles *self = o_user(view);
...
static void dir_home(oobj click)
{
...
static void edit_file_ok(oobj scene)
{
...
static void edit_file_cancel(oobj scene)
{
    // noop
}

static void edit_file(oobj click)
{
    XViewFiles *self = o_user(click);
...
static void show_prompt(XViewFiles *self, const char *prompt)
{
...
XViewFiles *XViewFiles_init(oobj obj, oobj parent, const char *title)
{
    ...
    XViewFiles_dir_home_set(self, o_file_home());
    self->dir_stack = OList_new(self, NULL, 0);

    self->tex_clear_full = true;
    ...
    oobj box = WBox_new(gui, WBox_V_WEIGHTS);
    WBox_spacing_set(box, vec2_(4));
    WObj_padding_set(box, vec4_(4));

    oobj title_align = WAlign_new_center_h(box);
...
void XViewFiles__v_update(oobj view, oobj tex, float dt)
{
    ...
    WTheme_update(self->w.theme, self->w.gui, lt, size);
...
void XViewFiles__v_render(oobj view, oobj tex, float dt)
{
    ...
    const vec4 bg_color = vec4_(0.6, 0.5, 0.4, 1.0);
    if (self->tex_clear_full) {
        RTex_clear(tex, bg_color);
    } else {
        RTex_clear(tex, bg_color);
    }
...
char *XViewFiles_dir_small(oobj obj)
{
    ...
    return o_str_join(obj, (char **) OList_list(stack), o_num(stack), "/");
}
