...
static void draw_gradients(XWObjColor *self, oobj theme)
{
    bvec4 gradient[32];
    vec3 hsv;
    vec4 rgba;
...
//    RTex_write_file(theme_tex, "#testcustom.png");
...
static void on_color_hsv_update(oobj gradient)
{
...
static void on_color_rgba_update(oobj gradient)
{
    XWObjColor *self = o_user(gradient);
    vec3 rgb;
...
static void hex_scene_ok(oobj scene)
{
...
static void hex_scene_cancel(oobj scene)
{
    // noop
}

static void on_hex_pressed(oobj click)
{
    ...
    XViewKeys_page_set(keys, XViewKeys_factory_page_hex(keys, NULL, NULL));
    XViewKeys_opt_keyboard_filter_set(keys, XViewKeys_factory_filter_hex);
...
XWObjColor *XWObjColor_init(oobj obj, oobj parent, vec4 uv_rect, vec4 rgba_prev, bool enable_alpha)
{
    ...
    oobj col_box = WBox_new_h(self->box);
...
vec2 XWObjColor__v_update(oobj obj, vec2 lt, vec2 min_size, bool enabled, oobj theme, a_pointer__fn pointer_fn)
{
    ...
    return WObj__v_update(obj, lt, min_size, enabled, theme, pointer_fn);
}
