...
XViewText *XViewText_init(oobj obj, oobj parent, const char *title, bool show_cancel, int max_chars)
{
    ...
    self->viewkeys = XViewKeys_new_text(self, NULL, NULL);
    ...
    oobj align = WAlign_new_center(self);
    self->w.gui = align;
    ...
    if (o_utils_clipboard_system_available()) {
...
void XViewText__v_update(oobj view, oobj tex, float dt)
{
    ...
    if (self->w.copy && WBtn_clicked(self->w.copy)) {
        o_utils_clipboard_set(OArray_data(self->text_array, char));
    }
    if (self->w.paste && WBtn_clicked(self->w.paste)) {
        char *paste = o_utils_clipboard_get(self);
        for (char *it = paste; it&&*it; it++) {
...
void XViewText__v_render(oobj view, oobj tex, float dt)
{
    ...
    if (self->tex_clear_full) {
        RTex_clear_full(tex, self->bg);
    } else {
        RTex_clear(tex, self->bg);
    }
...
void XViewText_custom_clear(oobj obj)
{
    ...
    self->w.custom_box = WBox_new(self->w.custom_box_container, WBox_H_V);
...
static void on_custom_btn_clicked(oobj obj)
{
...
void XViewText_custom_replace(oobj obj, const char *text, const vec4 *opt_color)
{
    ...
    // using custom box, so applying min_size...
    WObj_min_size_set(self->w.custom_box, vec2_(172, 0));
    ...
    struct custom_btn_context *C = o_user_new0(btn, *C, 1);
    C->self = self;
