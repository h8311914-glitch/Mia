...
static GLuint shader_stage_new(char **shader_codes, GLint shader_type, const char *source_tag_of_code)
{
    ...
    platform = "#define MIA_PLATFORM_MSVC\n"
               ...
               "#define MIA_PLATFORM_DESKTOP\n";
#endif
#ifdef MIA_PLATFORM_LINUX
    platform = "#define MIA_PLATFORM_LINUX\n"
               "#define MIA_PLATFORM_DESKTOP\n";
#endif
    ...
    osize shader_num = sclen + add_len;

    char *merged = r_program_merge_code(container, shader, &shader_num);

    glShaderSource(res, 1, (const GLchar * const *) &merged, NULL);
    glCompileShader(res);
...
static GLuint program_new(const GLuint *shaders, int n, const char *source_tag_of_code)
{
