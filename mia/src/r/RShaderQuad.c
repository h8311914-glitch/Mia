...
RShaderQuad* RShaderQuad_new_rgba(oobj parent, oobj tex, bool move_tex)
{
    ...
    RShader_program_set(self, r_program_Quad_rgba());
    return self;
}
...
{
    RShaderQuad *self = RShaderQuad_new(parent, tex, move_tex);
    RShader_program_set(self, r_program_Quad_rgba_hsva());
    return self;
}
...
void RShaderQuad__v_render(oobj obj, oobj bound_tex, oobj program, int num, const struct r_proj *proj)
{
    ...
    RProgram_uniform_vec4(program, "u_fy", &self->fy, 1);
    RProgram_uniform_vec4(program, "u_fz", &self->fz, 1);
