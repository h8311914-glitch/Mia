...
oobj r_tex_kernel_block2(bool normalized)
{
    ...
    if (normalized) {
        if (!lazy_n) {
            lazy_n = RTex_new_kernel(tex_L_o(), 2, 2, 1.0f, false, true, R_FORMAT_R_F32);
            ODelcallback_new_assert(lazy_n, "r_tex_kernel_block2", "deleted!");
        }
        return lazy_n;
    }
    if (!lazy) {
        lazy = RTex_new_kernel(tex_L_o(), 2, 2, 1.0f, false, false, R_FORMAT_R_F32);
...
oobj r_tex_kernel_block3(bool normalized)
{
    ...
    if (normalized) {
        if (!lazy_n) {
            lazy_n = RTex_new_kernel(tex_L_o(), 3, 3, 1.0f, false, true, R_FORMAT_R_F32);
            ODelcallback_new_assert(lazy_n, "r_tex_kernel_block3_n", "deleted!");
        }
        return lazy_n;
    }
    if (!lazy) {
        lazy = RTex_new_kernel(tex_L_o(), 3, 3, 1.0f, false, false, R_FORMAT_R_F32);
...
oobj r_tex_kernel_plus3(bool normalized)
{
    ...
    if (normalized) {
        if (!lazy_n) {
            lazy_n = RTex_new_kernel(tex_L_o(), 3, 3, 1.0f, true, true, R_FORMAT_R_F32);
            ODelcallback_new_assert(lazy_n, "r_tex_kernel_plus3_n", "deleted!");
        }
        return lazy_n;
    }
    if (!lazy) {
        lazy = RTex_new_kernel(tex_L_o(), 3, 3, 1.0f, true, false, R_FORMAT_R_F32);
...
oobj r_tex_kernel_gauss9(bool normalized)
{
    ...
    if (normalized) {
        if (!lazy_n) {
            lazy_n = RTex_new_kernel_gauss(tex_L_o(), 9, 9, vec2_(-1), true, R_FORMAT_R_F32);
            ODelcallback_new_assert(lazy_n, "r_tex_kernel_gauss9_n", "deleted!");
        }
        return lazy_n;
    }
    if (!lazy) {
        lazy = RTex_new_kernel_gauss(tex_L_o(), 9, 9, vec2_(-1), false, R_FORMAT_R_F32);
...
{\
    static RTex *lazy = NULL;                \
    if(!lazy) {                              \
        lazy = RTex_new_file(tex_L_o(), "$r/" #file ".png"); \
...
r_tex_DECL(font55)
r_tex_DECL(font55_shadow)
r_tex_DECL(font58)
r_tex_DECL(font58_shadow)
