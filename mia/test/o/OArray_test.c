...
static void test_default(oobj obj)
{
    ...
    test(OArray_available_front(a) == 0);
    for(int i=0; i<16; i++) {
        char val = 'A'+i;
...
static int test_sort_cmp(void *user_data, const void *a, const void *b)
{
    int ai = *(int*) a;
    int bi = *(int*) b;
    return ai - bi;
...
static void test_sort(oobj obj)
{
    oobj a = OArray_new(obj, NULL, sizeof(int), 0);
    for(int i=0; i<16; i++) {
        int random = o_rand() % 128;
        OArray_push(a, &random);
    }
    test(o_num(a) == 16);
    OArray_sort(a, test_sort_cmp);
    int *data = OArray_data_void(a);
    int prev = data[0];
    for(int i=1; i<16; i++) {
        int val = data[i];
        test(val >= prev);
...
static void test_copy(oobj obj)
{
...
static void test_struct(oobj obj)
{
    struct test {
        char c;
        int i;
...
static void test_take(oobj obj)
{
    oobj a = OArray_new(obj, NULL, sizeof (int), 0);
    for(int i=0; i<5; i++) {
        OArray_push(a, &i);
