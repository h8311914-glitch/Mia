...
static void test_match(oobj pattern, const char *str, struct o_str_range res,
...
int OPattern__test(oobj obj)
{
    ...
    test(pattern->groups[0].mode == OPattern_group_MUL);
    for (int i = 0; i < 256; i++) {
    ...
    oobj match_all_pattern = OPattern_new(obj, ".*");
    test(OPattern_match(match_all_pattern, "Hello World", NULL));

    oobj date_pattern = OPattern_new(obj, "%d%d%d%d%-%d%d%-%d%dT%d%d:%d%d:%d%d.*");
    test(OPattern_match(date_pattern, "2026-01-17T09:46:55+01:00", NULL));
    
    oobj match_all_foo_pattern = OPattern_new(obj, ".*Foo");
    test(!OPattern_match(match_all_foo_pattern, "Hello World", NULL));
    test(OPattern_match(match_all_foo_pattern, "Hello Foo", NULL));
