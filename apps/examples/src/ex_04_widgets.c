...
struct context {
     ...
     * While updating, all WObj widgets will add their render boxes into the themes box list.
    ...
    /**
     * We switch WBtn_style from a given list.
     * This is the index for that list.
     */
    int used_style;
     ...
     * The font is also part of the theme, so once again: ->single draw call for all widgets<-
     */
    oobj info;
...
static void setup(oobj view)
{
    ...
    /**
     * Initially sets the button style
     */
    WBtn_style_set(C->btn, WBtn_FLAT);
     ...
     */
    oobj box = WBox_new_h(C->btn);

    /**
     * Each WObj and its subclasses may change the padding (default is 0).
    ...
    /**
     * Sets the play icon's color green
     */
     ...
     * We can use padding to set a fixed position for the text "left 0; top 32 (so below...)"
     */
    C->info = WTextShadow_new(C->gui, NULL);
    WObj_padding_set(C->info, vec4_(0, 32, 0, 0));
}
...
static void update(oobj view, oobj tex, float dt)
{
     ...
     * @param lt left top start for the widgets. The widget system pops up to right bottom, as usual.
    ...
    /**
     * Switch between thoose btn styles.
     * WBtn defines a few more like:
     *    checkbox, radio button, dual buttons
     */
    enum WBtn_style styles[] = {
        WBtn_FLAT,
        WBtn_FLAT_ROUND,
        WBtn_DEF,
        WBtn_DEF_ROUND,
    ...
    if (WBtn_clicked(C->btn)) {
        C->used_style++;
        C->used_style %= o_stack_array_num(styles);
    }
    
    /**
     * Update the btn style
     */
    WBtn_style_set(C->btn, styles[C->used_style]);


    /**
     * o_strf(_a) creates a new formatted string (will get auto deleted at frame via the o_arena object)
     * NOTE: For this mini example, a stack buffer ala char buf[64] would have also been enough)
     *            can also be used with o_strf_buf
     *       The default o_arena allocator is in fact an arena allocator, so this is not a big of a deal.
     *       If you need another frame based object or resource with a bigger size (or unknown) have a look at:
     *            a_tmp() which returns a frame based OObj which is just a child of a_root() (pool allocator)
     */
    char *text = o_strf_a("USED BTN STYLE:\n%s", styles_text[C->used_style]);
...
static void render(oobj view, oobj tex, float dt)
{
     ...
     * Using RTex_clear_full instead of RTex_clear to also set the unsafe regions background color
     */
    RTex_clear_full(tex, R_GRAY_X(0.6));
 ...
 * It's straightforward by design, the next example "windows" will show much more widget stuff.
